<div>
    <form action="/dashboard/category" method="POST">
        ...
        @if(session('message'))
            <p>{{ session('message') }}</p>
        @endif
        <input type="text" name="category_id" id="categoryId" value="{{ request()->old('category_id', '') }}" autocomplete="off" placeholder="id kategori">
        <br>
        ...
        @else
            @foreach($category as $item)
                <tr>
                    ...
                    <td><a href="/dashboard/category/delete/{{ $item->category_id }}" onclick="return confirm('apakah anda yakin?')">hapus</a> <a href="/dashboard/category/update-category/{{ $item->category_id }}">edit</a></td>
                </tr>
            @endforeach
        @endif
    </table>
